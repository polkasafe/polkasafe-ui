...
export default async function verifyEmail(args: Args) {
	...
	if (!email || !verifyUrl || !validator.isURL(verifyUrl) || !validator.isEmail(email)) {
	...
	const { htmlMessage, markdownMessage, textMessage } = await getTemplateRender(SOURCE, TRIGGER_NAME, {
		...args,
		verifyUrl
	});
