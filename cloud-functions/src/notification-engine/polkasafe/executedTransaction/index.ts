...
const TRIGGER_NAME = 'executedTransaction';
...
export default async function executedTransaction(args: Args) {
	...
	for (const address of substrateAddresses) {
		...
		if (addressData) {
			...
			const link = `/transactions?tab=History#${callHash}&network=${network}&multisigAddress=${multisigAddress}`;
