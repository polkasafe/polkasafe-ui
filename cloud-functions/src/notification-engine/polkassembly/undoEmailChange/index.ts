...
const TRIGGER_NAME = 'undoEmailChange';
...
export default async function undoEmailChange(args: Args) {
	if (!args) throw Error(`Missing arguments for trigger: ${TRIGGER_NAME}`);
	const { email, undoUrl } = args;

	if (!email || !undoUrl || !validator.isURL(undoUrl) || !validator.isEmail(email)) throw Error(`Invalid arguments for trigger: ${TRIGGER_NAME}`);
