		...
		if (response.data && response.data.multisig?.length) {
			for (const multisigQueueItem of response.data.multisig) {
				...
				const newQueueItem: IQueueItem = {
					...
					transactionFields: transactionDoc.exists && transaction?.transactionFields ? transaction?.transactionFields : { category: 'none', subfields: {} },
					...
					notifications: transactionDoc.exists && transaction.notifications ? Object.fromEntries(
