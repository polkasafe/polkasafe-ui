								...
								return (
									<Link to={`/transactions?tab=Queue#${transaction.callHash}`} key={i} className="flex items-center justify-between pb-2 mb-2">
										...
										<div>
											...
											{!isNaN(Number(amountUSD)) && decodedCallData?.args?.value && <p className='text-white text-right text-xs'>{(Number(amountUSD) * Number(parseDecodedValue({ network, value: String(decodedCallData?.args?.value), withUnit: false }))).toFixed(2) || '0'} USD</p>}
										</div>
									</Link>
								);})
							:
